...
uint8_t cur_input2[IO_SIZE] = {255,};
...
static void (*client_browser_callback)(uint16_t,uint16_t,uint16_t,uint8_t); // the fields are: uint16_t webstatuscode,uint16_t datapos,uint16_t len; datapos is start of http data and len the the length of that data
...
uint8_t www_client_internal_result_callback(uint8_t fd, uint8_t statuscode, uint16_t datapos, uint16_t len_of_data){
        ...
        uint8_t c_input=255; 
        if (fd!=www_fd){
                (*client_browser_callback)(500,0,0,255);
                return(0);
        }
        if (statuscode==0 && len_of_data>12){
			...
			//for ( i = 0; i < IO_SIZE; i++ )
			for ( i = IO_SIZE - 1; i >= 0; i-- )
			{
				if (cur_input2[i] == www_fd + i)
				{
					c_input = i;
					cur_input2[i] = 255;
				}
			}
                        (*client_browser_callback)(web_statuscode,datapos,len_of_data, c_input);
                }
        }
        return(1);
}
...
//
void client_browse_url(const char *urlbuf,const char *urlbuf_varpart,const char *hoststr,void (*callback)(uint16_t,uint16_t,uint16_t,uint8_t),uint8_t *dstip,uint8_t *dstmac,uint16_t srvport)
{
        ...
        www_fd=client_tcp_req(&www_client_internal_result_callback,&www_client_internal_datafill_callback,srvport,dstip,dstmac);
	cur_input2[cur_input] = www_fd + cur_input;
}
...
/*
void client_http_post(const char *urlbuf, const char *urlbuf_varpart,const char *hoststr, const char *additionalheaderline,char *postval,void (*callback)(uint16_t,uint16_t,uint16_t,uint8_t),uint8_t *dstip,uint8_t *dstmac)
{
        if (!enc28j60linkup())return;
        client_urlbuf=urlbuf;
        client_hoststr=hoststr;
...
uint16_t packetloop_arp_icmp_tcp(uint8_t *buf,uint16_t plen)
{
        ...
        if ( buf[TCP_DST_PORT_H_P]==TCPCLIENT_SRC_PORT_H){
                ...
                if (tcp_client_state==3 && len>0){ 
                        ...
                        if (send_fin){
                                make_tcp_ack_from_any(buf,len,TCP_FLAGS_FIN_V);
                                tcp_client_state=5;
                                return(0);
                        }
                }
                if(tcp_client_state==5 && buf[TCP_FLAGS_P] & TCP_FLAGS_FIN_V) {
                        make_tcp_ack_from_any(buf,len,TCP_FLAGS_ACK_V);
			tcp_client_state=6;
                        // no more ack
                        return(0);
                }
